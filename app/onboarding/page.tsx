...
export default function OnboardingPage() {
  ...
  const [selectedClass, setSelectedClass] = useState<BusinessClassId | null>(null);
  const [selectedAvatar, setSelectedAvatar] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const selectedClassData = selectedClass ? BUSINESS_CLASSES[selectedClass] : null;
  ...
  }, [step, displayName, selectedClass]);
      ...
      <AnimatePresence mode="wait" custom={direction}>
        ...
        {step === 1 && (
          <motion.div
            ...
            exit="exit"
            transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] as const }}
            className="relative rounded-2xl border border-white/[0.06] p-6 md:p-8 backdrop-blur-xl overflow-hidden"
            style={glassCardStyle}
          >
            ...
            <motion.button
              onClick={handleBack}
              className="w-full mt-3 px-6 py-2.5 rounded-lg font-display uppercase tracking-wider text-white/70 text-sm
        ...
        {/* Step 3: First quest preview — class-specific quests */}
        {step === 3 && (
          <motion.div
            key="step-3"
            custom={direction}
          ...
          >
            ...
            >
              {selectedClassData ? (
                <>
                  <span className="mr-2">{selectedClassData.emoji}</span>
                  Tes qu&ecirc;tes de {selectedClassData.name}
                </>
              ) : (
            ...
            {/* Class-specific quest cards */}
            {selectedClassData ? (
              <div className="space-y-3 mb-6">
                {selectedClassData.quests.map((quest, qi) => (
                  <motion.div
                    key={quest}
                    className="rounded-xl border p-4 text-left overflow-hidden relative"
                    style={{
                      borderColor: `${selectedClassData.color}25`,
                      background: `linear-gradient(135deg, ${selectedClassData.color}08 0%, rgba(20,20,20,0.95) 100%)`,
                    }}
                  ...
                  >
                    {/* Shimmer effect on first quest */}
                    {qi === 0 && (
                      <motion.div
                        className="absolute inset-0 pointer-events-none"
                        style={{
                          background: `linear-gradient(105deg, transparent 40%, ${selectedClassData.color}10 50%, transparent 60%)`,
                        }}
                        initial={{ x: '-100%' }}
                        animate={{ x: '200%' }}
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{
                          backgroundColor: `${selectedClassData.color}15`,
                          color: selectedClassData.color,
                          border: `1px solid ${selectedClassData.color}25`,
                        }}
                      >
                        {qi + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <div className="flex-1 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                            <div className="h-full rounded-full w-0" style={{ backgroundColor: selectedClassData.color }} />
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {[
                ...
                >
                  <div
                    ...
                    style={{
                      background: selectedClassData
                        ? `linear-gradient(135deg, ${selectedClassData.color}18 0%, ${selectedClassData.color}08 100%)`
                        : 'linear-gradient(135deg, rgba(255,191,0,0.12) 0%, rgba(255,157,0,0.06) 100%)',
                    }}
                  >
                    <span
                      className="font-display font-bold text-xs"
                      style={{ color: selectedClassData?.color ?? '#FFBF00' }}
                    >
                      {i + 1}
                    </span>
                  </div>
          ...
          >
            ...
            <div className="relative mb-6">
              ...
              <motion.div
                ...
                style={{
                  background: selectedClassData
                    ? `linear-gradient(135deg, ${selectedClassData.color}25 0%, ${selectedClassData.color}10 100%)`
              ...
              >
                <motion.div
                  animate={{
                    boxShadow: [
                      `0 0 15px ${selectedClassData?.color ?? 'rgba(255,191,0,1)'}50`,
                      `0 0 40px ${selectedClassData?.color ?? 'rgba(255,191,0,1)'}80`,
                      `0 0 15px ${selectedClassData?.color ?? 'rgba(255,191,0,1)'}50`,
                    ],
                  }}
                  transition={{ duration: 2, repeat: Infinity }}
                ...
                >
                  {selectedClassData ? (
                    <span className="text-5xl">{selectedClassData.emoji}</span>
                  ) : (
                    <svg
              ...
              {selectedClassData && (
                ...
                >
                  {selectedClassData.emoji} {selectedClassData.name} &mdash; {selectedClassData.tagline}
                </motion.p>
              )}
              <motion.p
